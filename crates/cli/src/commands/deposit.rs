//! Deposit command implementation for V1 and V2 vaults.

use alloy_primitives::{Address, U256};
use anyhow::{Context, Result};
use api::{VaultV1TransactionClient, VaultV2TransactionClient};
...
/// Run the deposit command for a V1 vault.
pub async fn run_v1_deposit(args: &DepositArgs) -> Result<()> {
    ...
    let balance = client.get_balance(asset, signer).await?;
    ...
    if let Some(approval_receipt) = client.approve_if_needed(asset, vault, amount).await? {
        ...
        println!(
            "  Gas Used:  {}",
            format_gas(approval_receipt.gas_used)
        );
    }
...
/// Run the deposit command for a V2 vault.
pub async fn run_v2_deposit(args: &DepositArgs) -> Result<()> {
    let vault: Address = args.vault.parse().context("Invalid vault address")?;
